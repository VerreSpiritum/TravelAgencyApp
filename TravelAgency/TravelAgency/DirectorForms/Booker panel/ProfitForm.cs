...
{
    public partial class ProfitForm : Form, IViewProfit
    {
        public ProfitForm()
        {
            InitializeComponent();
        ...
        private void AddToTable()
        {
            foreach (DataRow row in ListOfStaff.Rows)
            {
                ...
                DateTime dateEnd = Convert.ToDateTime(row[2]);

                staffInfoTable.Rows.Add(row[0], $"{date.Day}/{date.Month}/{date.Year}", $"{dateEnd.Day}/{dateEnd.Month}/{dateEnd.Year}");
            }
        }
