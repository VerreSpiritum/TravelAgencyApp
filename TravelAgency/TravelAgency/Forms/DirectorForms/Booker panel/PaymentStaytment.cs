...
{
    public partial class PaymentStaytment : Form, IViewPaymentStaytment
    {
        public PaymentStaytment()
        {
            InitializeComponent();
        ...
        private void AddToTable()
        {
            foreach (DataRow row in ListOfStaff.Rows) 
            {
                ...
                DateTime dateEnd = Convert.ToDateTime(row[7]);

                staffInfoTable.Rows.Add(row[0], row[1], row[2], row[3], row[4], row[5], $"{date.Day}/{date.Month}/{date.Year}", $"{dateEnd.Day}/{dateEnd.Month}/{dateEnd.Year}");
            }
        }
